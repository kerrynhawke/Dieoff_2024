...
# ✅ SELECT YEARS + MONTHS (EDIT THESE)
target_years = [2023, 2024]
target_months = list(range(1, 13))   # e.g. [1,6,7,8,13]

# ---------------------------------------------------------
# LOAD DATA
# ---------------------------------------------------------

print("Loading AGCD precipitation...")
...
for label, (start, end) in periods.items():
    ...
    outfile = os.path.join(output_dir, f"swwa_precip_clim_{label}.nc")
...
# ---------------------------------------------------------
# SELECT TARGET DATA (YEARS + MONTHS)
# ---------------------------------------------------------
...
raw_file = os.path.join(
    output_dir,
    f"swwa_precip_monthly_{min(target_years)}_{max(target_years)}.nc"
)
...
anom_file = os.path.join(
    output_dir,
    f"swwa_precip_anom_{min(target_years)}_{max(target_years)}.nc"
)
