...
for label, (start, end) in periods.items():
    ...
    # ✅ CHANGE: filename uses tmin
    outfile = os.path.join(output_dir, f"swwa_tmin_clim_{label}.nc")
...
raw_file = os.path.join(
    output_dir,
    f"swwa_tmin_monthly_{min(target_years)}_{max(target_years)}.nc"
)
...
anom_file = os.path.join(
    output_dir,
    f"swwa_tmin_anom_{min(target_years)}_{max(target_years)}.nc"
)
