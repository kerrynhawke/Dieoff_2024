...
# ---------------------------------------------------------
# GENERIC FUNCTION (TMAX / TMIN / VPD)
# ---------------------------------------------------------
...
vpd_anom, vpd_rm, vpd_colors = process_variable(
    vpd_monthly_file, vpd_clim_file, "vpd"   # ✅ variable name inside file
)
...
ax3.set_ylabel("Tmin (°C)", fontsize=label_fs)
ax3.tick_params(axis="x", bottom=False, labelbottom=False)

# ----------------------------- VPD15 ✅
ax4.bar(years, vpd_anom, color=vpd_colors)
ax4.plot(years, vpd_rm, color="black", linewidth=2)
ax4.axhline(0, color="gray")
ax4.set_ylabel("VPD (kPa, 3pm)", fontsize=label_fs)
ax4.set_xlabel("Year", fontsize=label_fs, labelpad=20)
...
# SAVE
# ---------------------------------------------------------
outfile = os.path.join(output_dir, "SWWA_rain_tmax_tmin_vpd15_anomaly_AGCD.png")
